...
+ (id)objectWithDictionary:(NSDictionary *)dictionary context:(NSManagedObjectContext *)context {
	...
	NSString *remoteID = [NSString stringWithFormat:@"%@", [dictionary objectForKey:[[self class] remoteIDField]]];
	
	// If there isn't a remoteID, we won't find the object. Return nil.
	if (!remoteID || [remoteID isEqualToString:@""]) {
		return nil;
	}
...
- (void)unpackDictionary:(NSDictionary *)dictionary {
	if (!self.isRemote) {
		self.remoteID = [dictionary objectForKey:[SSRemoteManagedObject remoteIDField]];
	}
